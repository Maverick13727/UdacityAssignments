/*
 Goal : make Pizzeria website run jank-free at 60 frames per second.
...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    var arr_elementWidth = [];
    var windowWidth = document.getElementById("randomPizzas").offsetWidth;
    var lst_pizzas = document.getElementsByClassName("randomPizzaContainer")
      var arrLen = lst_pizzas.length;
    for (var i = 0; i < arrLen ; i++) {
      var dx = determineDx (lst_pizzas[i], size,windowWidth);
      var newwidth = (lst_pizzas[i].offsetWidth + dx) + 'px';
      arr_elementWidth.push(newwidth)
    }

    for (var i = 0; i < arrLen; i++) {
      lst_pizzas[i].style.width = arr_elementWidth[i];
    }
  }
...
//Moving pizzasDiv outside the lopp to minimise DOM manipulation
...
// Moves the sliding background pizzas based on scroll position
 function updatePositions() {
  ...
  var items = document.getElementsByClassName('mover');   //Moving DOM manipulation outside the looping construct
  var itemLen = items.length
  var scrollTop = document.body.scrollTop;    //Moving DOM manipulation outside the looping construct
  //get the phase value
  for (var i = 0; i < itemLen; i++) {
    var phase = Math.sin((scrollTop / 1250) + (i % 5));
    arr_phase.push(phase)
  }
  //apply the phase value
  for (var i = 0; i < itemLen; i++) {
    items[i].style.left = items[i].basicLeft + 100 * arr_phase[i] + 'px';
  }
...
// updatePositions  callback for scroll event on viewport
window.addEventListener('scroll', function() {
  /* wrapped inside requestAnimationFrame API to minimise triggerring of other events in te renderng pipeline
  by the method javascript
  */
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var pizzaHeight = 100
  var s = 256;
  //calculating pizzas wrt screen height
  var n_pizzas = (window.screen.height/pizzaHeight) * cols;
